...
class CSDI_base(nn.Module):
    def __init__(self, 
        ...
        config_diff = { 'layers': layers,
                        'channels': channels,
                        'nheads': nheads,
                        'diffusion_embedding_dim': nheads,
    ...
    def time_embedding(self, pos, d_model=128):
        ...
        div_term = 1 / torch.pow(
            10, torch.arange(0, d_model, 2) / d_model
        ).to(self.device)
    ...
    def get_side_info(self, observed_times, cond_mask):
        ...
        if self.is_unconditional == False:
            side_mask = cond_mask.unsqueeze(1).to(self.device)  # (B,1,K,L).to('cpu')
...
#                     print(torch.cuda.memory_summary(device=None, abbreviated=True))
                    predicted = self.diffmodel(diff_input.to(self.device), side_info, torch.tensor([t]).to(self.device))

                    coeff1 = 1 / self.alpha_hat[t] ** 0.5
                    coeff2 = (1 - self.alpha_hat[t]) / (1 - self.alpha[t]) ** 0.5
                    current_sample = coeff1 * (current_sample - coeff2 * predicted.to('cpu'))
                    del predicted
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            ...
            print(f'type of obs data:', observed_data.dtype)
            print(f'type of cond mask:', cond_mask.dtype)
            print(f'type of obs times:', observed_times.dtype)
            print(f'type of side info:', side_info.dtype)
    ...
    def process_data(self, batch):
        ...
        try:
            ...
            observed_times = torch.from_numpy(np.asarray(batch["times"]).astype('long')).to(self.device).float()
        ...
        # observed_data = observed_data.permute(0, 3, 1)
        # observed_mask = observed_mask.permute(0, 3, 1)
        # gt_mask = gt_mask.permute(0, 3, 1)
